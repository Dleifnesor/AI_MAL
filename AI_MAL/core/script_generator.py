...
"""
    ...
    def _generate_ruby_service_enum(self, scan_results: Dict[str, Any]) -> str:
        r"""Generate Ruby service enumeration script."""
        try:
...
# Main execution
if ARGV.length != 2
  puts "Usage: #{$0} <target> <ports>"
  puts "Example: #{$0} 192.168.1.1 80,443,8080"
  exit 1
end
...
"""
            ...
            self.logger.error(f"Error generating Ruby service enumeration: {str(e)}")
...
"""
            ...
            self.logger.error(f"Error generating Ruby service enumerator: {str(e)}")
            return self._generate_fallback_script("service_enumerator", "ruby")
