...
class AI_MAL:
    ...
    def run(self, target: Optional[str] = None, scan_type: str = "basic", **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.config.get("custom_scripts"):
                try:
                    scripts = self.script_generator.generate_scripts(results)
   ...
   AI-Powered Penetration Testing Tool
    ...
    def _initialize_ai_models(self) -> None:
        ...
        try:
            ...
            if self.ai_manager.model in self.ai_manager.available_models:
                ...
                primary_model_available = self._pull_model(self.ai_manager.model)
    ...
    def _build_scan_config(self, target: str, scan_type: str, **kwargs) -> Dict[str, Any]:
        """
        Build scan configuration dictionary.
        ...
        config = {
            ...
            "interface": interface,
            "scan_type": scan_type,
            "ports": kwargs.get("ports", "1-65535"),  # Full port range by default
            ...
            "vuln_detection": kwargs.get("vuln_detection", False),
            "dos_testing": kwargs.get("dos_testing", False),
            "msf_enabled": kwargs.get("msf_enabled", False),
            "exploit_enabled": kwargs.get("exploit_enabled", False),
            "custom_scripts": kwargs.get("custom_scripts", False),
            "script_type": kwargs.get("script_type", "python"),
            "execute_scripts": kwargs.get("execute_scripts", False),
            "script_output": kwargs.get("script_output", "./scripts"),
            "script_format": kwargs.get("script_format", "raw"),
            ...
            "exfil": kwargs.get("exfil", False),
            "implant": kwargs.get("implant"),
            "output_dir": kwargs.get("output_dir", "./results"),
            "output_format": kwargs.get("output_format", "json"),
            "quiet": kwargs.get("quiet", False),
            "no_gui": kwargs.get("no_gui", False),
            "log_level": kwargs.get("log_level", "info"),
            "log_file": kwargs.get("log_file", "logs/AI_MAL.log"),
            "full_auto": kwargs.get("full_auto", False),
            "custom_vuln": kwargs.get("custom_vuln"),
            "min_parallelism": 10,
            "max_parallelism": 100,
            "scanflags": "SYN",
            ...
            # OpenVAS specific configurations
            "openvas": {
                "scan_config": kwargs.get("scan_config", "full_and_fast"),
                "gvm_user": kwargs.get("gvm_user", "admin"),
                "gvm_password": kwargs.get("gvm_password", None),
                "update_feeds": kwargs.get("update_feeds", False),
                "cleanup": kwargs.get("cleanup", True),
                "results_dir": kwargs.get("output_dir", "./results"),
                "fallback_to_nmap": kwargs.get("fallback_to_nmap", True),
            }
        }
        
        # Add scan type specific settings
        if scan_type == "stealthy" or scan_type == "stealth":
            config.update({
                "min_parallelism": 1,
                "max_parallelism": 10,
                "scanflags": "NULL",
        ...
        elif scan_type == "aggressive" or scan_type == "full":
            config.update({
                "min_parallelism": 50,
                "max_parallelism": 200,
                "scanflags": "SYN",
            ...
            # Specific OpenVAS scan type settings
            config.update({
                "vuln_detection": True,
    ...
    def _process_scan_results(self, scan_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Returns:
            Processed results including AI analysis if enabled.
        """
        try:
            ...
            # Perform AI analysis if enabled
            if self.ai_analysis and self.ai_manager.is_ollama_available():
                try:
            ...
            if self.exploit_generation:
                try:
                    exploits = self.msf_manager.generate_exploits(results)
                    results["exploits"] = exploits
                except Exception as e:
                    self.logger.error(f"Exploit generation failed: {str(e)}")
                    if self.verbose:
            ...
            error_msg = f"Error processing scan results: {str(e)}"
            self.logger.error(error_msg)
            if self.verbose:
                self.logger.exception("Full exception:")
            return {"error": error_msg}
